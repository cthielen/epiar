...
function showComponent(kind, name)
	...
	for i,layout in ipairs(EditorLayouts[kind]) do
		...
		elseif fieldType == "Sound" then
			...
			if value ~= "" then 
				-- Chop off the path part
				local subpath = "data/audio/"..subgroup.."/"
				value = value:sub( value:find(subpath) + string.len(subpath) )
				field:setText( value )
			end
			...
			function previewSound( subgroup, searchPath )
				...
				if dropdown then
					local sound = string.format("data/audio/%s/%s", subgroup, dropdown:GetText() )
	...
	for i,layout in ipairs(EditorLayouts[kind]) do
		...
		if texts[title]~=nil then
			...
			elseif fieldType == "Sound" then
				...
				info[title] = "data/audio/"..subgroup.."/" .. texts[title]:GetText()
			elseif fieldType == "Animation" then
				info[title] = "data/animations/" .. texts[title]:GetText()
			elseif fieldType == "Technologies" then
				local techs = {}
...
function ImagePicker(name,title)
	...
	for i,picPath in pairs( Epiar.listImages()) do
		imagePickerWin:add(
			UI.newPicture(25,25+300*(i-1),200,200,"data/graphics/"..picPath),
			UI.newButton( 25,225+300*(i-1),200,30, picPath,string.format("imagePick(%q,%q,%q)",name,title,"data/graphics/"..picPath )))
	end
end
